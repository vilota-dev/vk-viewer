...
class CapnpSubscriber(MessageSubscriber):
  ...
  def _on_receive(self, topic_name, msg_raw, time):
    if self.typeclass is None:
      self.callback(self.topic_type, topic_name, msg_raw, time)
    else:
      with self.typeclass.from_bytes(msg_raw) as msg:
        self.callback(self.topic_type, topic_name, msg, time, msg_raw)
